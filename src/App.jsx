...
export default function App() {
  ...
  function submitKey() {
    if (!keyInput.trim().startsWith("gsk_")) {
      setKeyError("Key should start with gsk_ — check and try again");
      return;
    }
    ...
    try {
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 300,
          messages: [
            { role: "system", content: buildSystemPrompt(companion, mode) },
            ...newMessages
          ],
        }),
      });
  ...
  if (screen === "key") return (
    <div style={styles.shell}>
      <div style={styles.inner}>
        <div style={styles.logoRow}><span style={styles.logoText}>echo<span style={styles.logoDot}>.</span></span></div>
        <p style={styles.tagline}>your AI companion, always there</p>
        <div style={styles.keyBox}>
          <p style={styles.keyTitle}>Enter your Groq API Key</p>
          <p style={styles.keyDesc}>Get a free key at console.groq.com → API Keys</p>
          <input style={styles.keyInput} type="password" placeholder="gsk_..."
            value={keyInput} onChange={e => { setKeyInput(e.target.value); setKeyError(""); }} />
          {keyError && <p style={styles.keyError}>{keyError}</p>}
          <button style={{ ...styles.cta, background: "#c084fc", marginTop: 8 }} onClick={submitKey}>Enter Echo →</button>
        </div>
        <p style={styles.keyNote}>🔒 Your key stays on your device only</p>
      </div>
    </div>
  );
    ...
    <div style={styles.shell}>
      <div style={styles.inner}>
        ...
        <button style={{ ...styles.cta, opacity: companion ? 1 : 0.4, background: companion ? companion.color : "#555" }}
          disabled={!companion} onClick={() => setScreen("mode")}>Continue →</button>
      </div>
    </div>
  );
    ...
    <div style={styles.shell}>
      <div style={styles.inner}>
        ...
        <button style={{ ...styles.cta, opacity: mode ? 1 : 0.4, background: mode ? companion.color : "#555" }}
          disabled={!mode} onClick={() => startChat(mode)}>Start talking →</button>
      </div>
    </div>
  );
    ...
    <div style={styles.shell}>
      <div style={styles.chatWrap}>
        ...
        <div style={styles.inputRow}>
          <textarea style={styles.inputBox} value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={handleKey} placeholder={`Message ${companion.name}...`} rows={1} />
...
const styles = {
  shell: { minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia', serif" },
  ...
  keyBox: { width: "100%", background: "#111", borderRadius: 16, padding: "24px", display: "flex", flexDirection: "column", gap: 12 },
  keyTitle: { color: "#fff", fontSize: 16, fontWeight: "bold", margin: 0 },
  keyDesc: { color: "#555", fontSize: 12, margin: 0, fontStyle: "italic" },
  keyInput: { background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 10, color: "#fff", padding: "12px 14px", fontSize: 14, outline: "none", fontFamily: "Georgia, serif", width: "100%" },
  keyError: { color: "#f87171", fontSize: 12, margin: 0 },
  keyNote: { color: "#333", fontSize: 11, textAlign: "center" },
  cardGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, width: "100%" },
  card: { background: "#111", border: "1.5px solid transparent", borderRadius: 16, padding: "20px 12px", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer", transition: "all 0.2s", position: "relative" },
  ...
  cta: { width: "100%", padding: "16px", borderRadius: 14, border: "none", color: "#000", fontSize: 15, fontWeight: "bold", cursor: "pointer", transition: "all 0.2s", marginTop: 8 },
  ...
  modeCard: { border: "1.5px solid", borderRadius: 14, padding: "16px 18px", cursor: "pointer", display: "flex", flexDirection: "column", gap: 4, transition: "all 0.2s", textAlign: "left" },
  ...
  chatHeader: { padding: "16px 20px", background: "#0f0f0f", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", gap: 12 },
  chatName: { fontSize: 16, fontWeight: "bold" },
  chatStatus: { color: "#4ade80", fontSize: 11 },
  modeBadge: { marginLeft: "auto", fontSize: 18 },
  messages: { flex: 1, overflowY: "auto", padding: "20px 16px", display: "flex", flexDirection: "column", gap: 10 },
  bubble: { maxWidth: "78%", padding: "12px 16px", borderRadius: 18, fontSize: 14, lineHeight: 1.6, wordBreak: "break-word" },
  inputRow: { padding: "12px 16px", background: "#0f0f0f", borderTop: "1px solid #1a1a1a", display: "flex", gap: 10, alignItems: "flex-end" },
